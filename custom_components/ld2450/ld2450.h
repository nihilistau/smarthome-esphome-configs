...
// The stock buffer (41 bytes) breaks on newer firmware that appends extended
// debug sections and BLE metadata. Bump the ceiling so we can safely parse the
// longer frames without dropping them. 4 KB gives us plenty of headroom even
// when the module emits BLE info, diagnostics, and reserved padding in a single
// shot.
static constexpr size_t MAX_LINE_LENGTH = 4096;          // Max characters for serial buffer
static constexpr size_t RAW_STREAM_CAPTURE_BYTES = 256;  // First bytes to capture verbatim for diagnostics
static constexpr size_t RAW_STREAM_CAPTURE_LOG_THRESHOLD = 16;
static constexpr uint32_t RAW_STREAM_CAPTURE_TIMEOUT_MS = 5000;
static constexpr uint8_t HEADER_FOOTER_SIZE = 4;         // Bytes in LD24xx frame headers/footers
...
class LD2450Component : public Component, public uart::UARTDevice {
 ...
 protected:
  ...
  std::array<uint8_t, RAW_STREAM_CAPTURE_BYTES> raw_stream_capture_{};
  uint8_t mac_address_[6] = {0, 0, 0, 0, 0, 0};
  uint8_t version_[6] = {0, 0, 0, 0, 0, 0};
  size_t buffer_pos_ = 0;    // where to resume processing/populating buffer
  uint32_t periodic_frame_log_counter_{0};
  uint32_t header_candidate_log_counter_{0};
  uint16_t raw_stream_capture_pos_{0};
  bool raw_stream_capture_logged_{false};
  uint32_t raw_stream_capture_start_ms_{0};
  bool raw_stream_waiting_logged_{false};
  enum class FrameType : uint8_t {
    FRAME_NONE = 0,
    FRAME_PERIODIC,
    FRAME_ACK,
  };
  FrameType frame_type_{FrameType::FRAME_NONE};
  std::array<uint8_t, HEADER_FOOTER_SIZE> header_window_{};
  size_t header_window_size_{0};
