...
#endif
  this->raw_stream_capture_pos_ = 0;
  this->raw_stream_capture_logged_ = false;
  this->raw_stream_waiting_logged_ = false;
  this->raw_stream_capture_start_ms_ = millis();
...
void LD2450Component::loop() {
  if (!this->raw_stream_capture_logged_ && !this->raw_stream_waiting_logged_ &&
      (millis() - this->raw_stream_capture_start_ms_) > 3000 && this->raw_stream_capture_pos_ == 0) {
    ESP_LOGW(TAG, "Still waiting for initial UART bytes from radar (no data seen yet)");
    this->raw_stream_waiting_logged_ = true;
  }
...
void LD2450Component::handle_periodic_data_() {
  ...
  if ((this->periodic_frame_log_counter_++ % 10) == 0) {
    ESP_LOGI(TAG, "Periodic frame (%u bytes): %s", static_cast<unsigned>(this->buffer_pos_),
             format_hex_pretty(this->buffer_data_, this->buffer_pos_).c_str());
  }
...
void LD2450Component::readline_(int readch) {
  ...
  if (!this->raw_stream_capture_logged_ && this->raw_stream_capture_pos_ < RAW_STREAM_CAPTURE_BYTES) {
    this->raw_stream_capture_[this->raw_stream_capture_pos_++] = static_cast<uint8_t>(readch);
  }

  if (!this->raw_stream_capture_logged_) {
    const bool threshold_hit = this->raw_stream_capture_pos_ >= RAW_STREAM_CAPTURE_LOG_THRESHOLD;
    const bool timeout_elapsed =
        (this->raw_stream_capture_pos_ > 0) &&
        ((millis() - this->raw_stream_capture_start_ms_) >= RAW_STREAM_CAPTURE_TIMEOUT_MS);
    if (threshold_hit || timeout_elapsed) {
      const size_t bytes_to_log = this->raw_stream_capture_pos_;
      ESP_LOGI(TAG, "Initial raw stream sample (%u bytes): %s", static_cast<unsigned>(bytes_to_log),
               format_hex_pretty(this->raw_stream_capture_.data(), bytes_to_log).c_str());
      this->raw_stream_capture_logged_ = true;
    }
  }

  auto reset_header_window = [this]() { this->header_window_size_ = 0; };

  if (this->frame_type_ != FrameType::FRAME_NONE) {
    if (this->buffer_pos_ >= MAX_LINE_LENGTH - 1) {
      ESP_LOGW(TAG, "Frame exceeded buffer (>%u bytes); dropping %u bytes", MAX_LINE_LENGTH, this->buffer_pos_);
      this->buffer_pos_ = 0;
      this->frame_type_ = FrameType::FRAME_NONE;
      reset_header_window();
      return;
    }

    this->buffer_data_[this->buffer_pos_++] = static_cast<uint8_t>(readch);

    if (this->frame_type_ == FrameType::FRAME_PERIODIC) {
      if (this->buffer_pos_ >= 2 && this->buffer_data_[this->buffer_pos_ - 2] == DATA_FRAME_FOOTER[0] &&
          this->buffer_data_[this->buffer_pos_ - 1] == DATA_FRAME_FOOTER[1]) {
    ...
    } else if (this->buffer_pos_ >= HEADER_FOOTER_SIZE &&
               ld2450::validate_header_footer(CMD_FRAME_FOOTER,
                                              &this->buffer_data_[this->buffer_pos_ - HEADER_FOOTER_SIZE])) {
  ...
  // Not currently in a frame: maintain a sliding header window until we find the next frame start
  if (this->header_window_size_ < HEADER_FOOTER_SIZE) {
    this->header_window_[this->header_window_size_++] = static_cast<uint8_t>(readch);
  } else {
    std::memmove(this->header_window_.data(), this->header_window_.data() + 1, HEADER_FOOTER_SIZE - 1);
    this->header_window_[HEADER_FOOTER_SIZE - 1] = static_cast<uint8_t>(readch);
  }

  if (this->header_window_size_ == HEADER_FOOTER_SIZE) {
    const uint8_t *window = this->header_window_.data();
    auto should_log_header = [this]() { return (this->header_candidate_log_counter_++ % 50) == 0; };
    if (window[0] == 0xAA && window[1] == 0xFF && should_log_header()) {
      ESP_LOGI(TAG, "Header candidate (AA FF ..): %s", format_hex_pretty(window, HEADER_FOOTER_SIZE).c_str());
    }
    if (window[0] == 0xFD && window[1] == 0xFC && should_log_header()) {
      ESP_LOGI(TAG, "Header candidate (FD FC ..): %s", format_hex_pretty(window, HEADER_FOOTER_SIZE).c_str());
    }
    if (std::memcmp(window, DATA_FRAME_HEADER, HEADER_FOOTER_SIZE) == 0) {
      std::memcpy(this->buffer_data_, DATA_FRAME_HEADER, HEADER_FOOTER_SIZE);
      this->buffer_pos_ = HEADER_FOOTER_SIZE;
      this->frame_type_ = FrameType::FRAME_PERIODIC;
      reset_header_window();
    } else if (std::memcmp(window, CMD_FRAME_HEADER, HEADER_FOOTER_SIZE) == 0) {
      std::memcpy(this->buffer_data_, CMD_FRAME_HEADER, HEADER_FOOTER_SIZE);
      this->buffer_pos_ = HEADER_FOOTER_SIZE;
      this->frame_type_ = FrameType::FRAME_ACK;
      reset_header_window();
    } else {
      // keep window sliding; no match yet
    }
  }
}
